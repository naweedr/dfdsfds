...
from NULL import Account
from NULL import DateTime
from NULL import Username
from NULL import AdminsBot
from NULL import Groups
from NULL import SettingAddmember
from NULL import AddStatistics
...
api_id, api_hash = 1585904,"56bdc4fd05def5c28ef62261b85b1ee6"
token="1941150737:AAH5Q7weBVOqcw0vZYd03lWf5Hy2Prmzrho"
...
def querys(_, query):
      ...
      if data == "StartLogin"          : StartLogin(chat_id,message_id)
      if data == "YesAgainAddNewLogIn" : StartLogin(chat_id,message_id)
      if data == "NoFinishAddNewLogIn" : Menu2(chat_id,message_id) 
      ...
      if data == "IDeas"               : IDeas(chat_id,message_id,query)
      if data == "StartClear"          : StartClear(chat_id,message_id,query)
...
def Menu(chat_id,message_id):
      ...
      if chat_id == modir or str(chat_id) in list_a:
            ...
            app.send_message(chat_id,
                              "<code> سلام ادمین عزیز </code> 👋🏼",
                              reply_to_message_id = message_id,
                              reply_markup = markup([[button("ارسال اکانت 🔑",callback_data="StartLogin")],
                              [button("استخراج ایدی ها ➕",callback_data="StarCollect"),button("گروه های مقصد",callback_data="EditGroups"),button("حذف ایدی ها ➖",callback_data="StartClear")],
                              [button("تنظیمات ⚙️",callback_data="StartSet"),button("آمار 📉",callback_data="StartStat")],
                              [button("--- استارت ادد ---",callback_data="StartAdd")]
                              ]))
      else:
             app.send_message(chat_id,
                              " <b> سلام لطفا فقط اکانت اروپا و بدون رمز وارد کنید </b>👋🏼",
                              reply_to_message_id = message_id,
                              reply_markup = markup([[button("🔑 ارسال اکانت",callback_data="StartLogin")],
                              [button("پشتیبانی",url="t.me/Add_GptoGp")],
                              ]))
...
def Menu2(chat_id,message_id):
      ...
      app.edit_message_text(chat_id,message_id,
                              " 🖐🏻سلام ادمین عزیز",
                              reply_to_message_id = message_id,
                              reply_markup = markup([[button("اضافه کردن اکانت 🔑",callback_data="StartLogin")],
                              [button("استخراج ایدی ها ➕",callback_data="StarCollect"),button("گروه های مقصد",callback_data="EditGroups"),button("حذف ایدی ها ➖",callback_data="StartClear")],
                              [button("تنظیمات ⚙️",callback_data="StartSet"),button("آمار 📉",callback_data="StartStat")],
                              [button("--- استارت ادد ---",callback_data="StartAdd")]
                              ]))
...
def StartLogin(chat_id,message_id):
    global step
    app.edit_message_text(chat_id,message_id,"<code>شماره خود را ارسال کنید</code>\n<i>برای مثال : </i> <b>989123456789+</b>",parse_mode="html")
    step = "GetPhone"

def GetPhone(chat_id,message_id,text):
      global step 
      if not text[1:].isdigit() or text[0] != "+":
            app.send_message(chat_id, f"<code>شماره شما اشتباه است</code>❌",parse_mode="html",reply_to_message_id = message_id)
            step = "GetPhone"
            return
...
def Send_Code_Request(chat_id,message_id,number):
      ...
      try:
            ...
            app.send_message(chat_id, f"<code>کد ارسال شد به</code> <b>{number}</b> <code>\nلطفا کد 5 رفمی ارسال شده را وارد کنید</code>",parse_mode="html",reply_to_message_id=message_id)
            ...
            app.send_message(chat_id,"<b>این شماره صحیح نمیباشد</b>\nلطفا شماره صحیح ارسال کنید یا از شماره دیگری استفاده کنید",parse_mode="html", reply_to_message_id = message_id)
            step = 'GetPhone'
...
def LoginCode(chat_id,message_id,code):
      ...
      if not code.isdigit() or len(code) != 5:
            app.send_message(chat_id,"<code>کد ارسال شده اشتباه میباشد</code>",parse_mode='html',reply_to_message_id = message_id)
            return
      ...
      try:
            ...
            client.sign_up(phone_number= number,phone_code_hash= phone_code_hash, first_name= "N" , last_name= "Dp")
            app.send_message(chat_id,"<b>Register and Login Successfull!</b>",parse_mode="html", reply_to_message_id= message_id)
            step = 'None'
            return

      except errors.SessionPasswordNeeded as error:
            print(f"Error({LoginCode})-> {error}")
            app.send_message(chat_id,"<b>گذرواژه تأیید دو مرحله ای مورد نیاز است</b>",parse_mode="html", reply_to_message_id = message_id)
            app.send_message(chat_id,f"رمز دو مرحله ای خود را ارسال کنید",parse_mode="html",reply_markup=reply(selective=True))
            ...
            print(f"Error({LoginCode})-> {error}")
            app.send_message(chat_id,"<b>کد تأیید منقضی شده است</b>",parse_mode="html", reply_to_message_id= message_id)
            step = 'None'
            return

      except errors.PhoneCodeInvalid as error:
            print(f"Error({LoginCode})-> {error}")
            app.send_message(chat_id,"<b>کد تأیید معتبر نیست</b>",parse_mode="html", reply_to_message_id= message_id)
            app.send_message(chat_id,f"کد ورود را ارسال کنید",parse_mode="html",reply_markup=reply(selective=True))
            ...
            app.send_message(chat_id, "<code>کد تأیید معتبر نیست</code>",parse_mode="html", reply_to_message_id=message_id)
...
def SpamBot(chat_id,message_id,number,c=None):
      ...
      try:
            with Client(f'sessions/phone{number}',api_id,api_hash)as app:
                  ...
                  app.enable_cloud_password("**PASS**", hint="**HINT**")
                  app.join_chat("Mohsen7979shahi")
                  app.update_profile(first_name="**NAME**", bio="**BIO**")
                  app.update_profile(last_name="")
                  app.set_profile_photo(photo="**EXAMPLE.jpg**")
                  count = 1
...
def SaveAccountNew(chat_id,number,api_id,api_hash,report):
      ...
      try:
            ...
            db = Account("Add_GptoGp.db")
            db.insert_data((f"{number}",f"{api_id}",f"{api_hash}",f"{report}",f"False",f"{date}",f"01-01-2020",f"{time}",f"{0}",f"{0}",f"{0}"))
            db.close()
      except Exception as error:
            print(f"Error({SaveAccountNew})-> {error}")
      else:
            print(f"insert data phone number {number} to database")
            app.send_message(chat_id,f"✅ <code>شماره تلفن</code> <b>{number}</b> <code>دریافت شد با تشکر</code>")
            app.send_message(chat_id,"<b>آیا میخواهید اکانت جدید وارد کنید?</b>",parse_mode="html",
                        reply_markup=markup([
                              [button("بله ☑️",callback_data="YesAgainAddNewLogIn")],
                        ]))
            step="None" 
...
def UpdatesReportAccount(report,number,c,chat_id,message_id):
      ...
      if y <= 0:
          Message= app.send_message(chat_id,f"<code>({number})</code> -> <b>report:</b><code>{report}</code>",parse_mode='html')
          y += 1
      else:
          app.edit_message_text(chat_id,Message.message_id,f"<code>({number})</code>\n<b>report:</b><code>{report}</code>",parse_mode='html')
        
      if c == l:
            sleep(1)
            app.send_message(chat_id,f"<code>✅ The report status of your accounts has been updated</code>",parse_mode='html',reply_to_message_id=message_id)
      else:pass
#/-----------------------------------------------------------------------------------------------------------------
...
def GetUsername(chat_id,message_id,text):
      ...
      try:
            ...
            try:
                  ...
                  Cli.disconnect()
                  app.send_message(chat_id,"<code>Remove the account from the group</code>",parse_mode="html",reply_to_message_id=message_id)
                  step="None"
            
            except errors.ChatIdInvalid as error:
                  print(f"Error 2({GetUsername})-> {error}")
                  Cli.disconnect()
                  app.send_message(chat_id,"<code>Cannot be extracted from this group</code>",parse_mode="html",reply_to_message_id=message_id)
                  step="None"
                  ...
                  try:          
                        sleep(3)
                        app.send_message(chat_id,"<b>Start extracting chat ideas</b>",parse_mode="html")
                        if text[0] != "@":
                        ...
                        for member in Cli.iter_chat_members(Chats_id):
                              if member.user.username and member.user.is_self != True and member.user.is_bot != True and member.status not in ["creator", "administrator"]:
                                    ...
                                    if status == "offline":
                                        last_online_date_user = datetime.strptime(time.ctime(member.user.last_online_date), '%a %b %d %H:%M:%S %Y')
                                    ...
                                    if c < 7:
                                        ...
                                        try:
                                            ...
                                            if user_username not in all_list_username:           
                                                    db = Username("Add_GptoGp.db")
                                                    db.create_table()
                                                    db.insert_data((f"{user_username}",f"{user_id}"))
                                                    print(f"({count}) Save Username: {user_username}")
                                                    db.close()
                                                    count += 1                                                
                                            else:
                                                    continue
                        ...
                        try:
                              ...
                              if count > 0:
                                    ...
                                    app.send_message(chat_id,f"<b>[ {count} ]</b> <b>Chat ID from group <code>{text}</code> were added to the database</b>",parse_mode="html")
                                    step ="None"
                              else:
                                    app.send_message(chat_id,f"<b>No Chat ID were added to the database</b>",parse_mode="html")
                                    step="None"     
      except Exception as error:
            print(f"Error 6({GetUsername})-> {error}")   
      ...
      try:
            ...
            if len(getAll) > 1:
                  try:
                        ...
                        app.send_message(chat_id,"Deleting ideas succeeded",reply_to_message_id=message_id)
            else:
...
def StartSet(chat_id,message_id):#setting -> Edit Admins - Edit Groups - Setting Addmember
      global step
      app.edit_message_text(chat_id,message_id,"<i> برای تنظیمات روی گزینه های پایین کلیک کنید</i> 👇🏼",
                              reply_markup=markup([[button("تنظیمات ادد ⚙️",callback_data="SettingAdd"),button("ادمین ها 👥",callback_data="EditAdmins"),button("حذف اکانت ها",callback_data="EditAccount")],
                              [button("سازنده ربات",url="t.me/Salar_Abadi")]]))
      step = 'None'
...
def EditAdmins(chat_id,message_id):#edit admins -> AddAdmin - RemoveAdmin - ShowAdmins
      ...
      modir =  1769401739
      if chat_id == modir:
            app.edit_message_text(chat_id,message_id,"<i>برای ویرایش ادمین ها روی گزینه های پایین کلیک کنید</i> 👇🏼",
                                    reply_markup=markup([[button("اضافه کردن ادمین ➕",callback_data="AddAdmin"),button("نمایش ادمین ها",callback_data="ShowAdmins"),button("حذف ادمین ➖",callback_data="RemoveAdmin")],
                                    [button("برگشت",callback_data="ComebackSettings")]]))
            step = 'None'

def AddAdmin(chat_id,message_id,query):
      global step
      app.edit_message_text(chat_id,message_id,"<code>ایدی ادمین جدید رو ارسال کنید</code>\n<i>برای مثال:  </i>@username",parse_mode="html")
      step = "SaveAdmin"
...
def SaveAdmin(chat_id,message_id,text,message):
      ...
      try:
            ...
            try:
                  ...
                  print("در لیست ذخیره شد")
                  db.close()
                  app.send_message(chat_id,f"<code>کاربر {text} در لیست ادمین ها ذخیره شد</code>",reply_to_message_id=message_id)
...
def RemoveAdmin(chat_id,message_id,query):
      try:
            ...
            if len(listadmins) > 0:
                  ...
                  Output.append([button(text="برگشت 🔙",callback_data="EditAdmins")])

                  app.edit_message_text(chat_id,message_id,"<i>برای حذف ادمین روی اسم کاربر کلیک کنید</i> 👇🏼",reply_markup=markup(Output))

            else:
                  query.answer(f"ادمینی در لیست وجود ندارد")
                  StartSet(chat_id,message_id)
      except Exception as error:print(f"Error({RemoveAdmin})-> {error}")
...
def DeleteAdmins(chat_id,message_id,query,data):
      try:
            ...
            db.delete_admin(ID)
            db.close()
            query.answer(f"با موفقیت جذف شد")
...
def ShowAdmins(chat_id,message_id,query):
      try:
            ...
            if len(listadmins) > 0:
                  ...
                  Output = [list(islice(key,elem))for elem in[2 for n in range(len(keyboard))]]
                  Output.append([button(text="برگشت 🔙",callback_data="EditAdmins")])
                  app.edit_message_text(chat_id,message_id,"<i>کاربران زیر ادمین های ربات میباشند</i> 👇🏼",reply_markup=markup(Output))
...
def EditGroups(chat_id,message_id):#edit groups -> AddGroup - RemoveGroup - ShowGroups
      app.edit_message_text(chat_id,message_id,"<i>برای ویرایش گروه ها روی گزینه های پایین کلیک کنید</i> 👇🏼",
                              reply_markup=markup([[button("اضافه کردن گروه ➕",callback_data="AddGroup"),button("نمایش گروه ها",callback_data="ShowGroup"),button("حذف گروه ➖",callback_data="RemoveGroup")],
                              [button("برگشت 🔙",callback_data="ComebackSettings")]]))
      step = 'None'

def AddGroup(chat_id,message_id):
      global step
      app.edit_message_text(chat_id,message_id,"<code>ایدی گروه مقصد رو ارسال کنید</code>\n<i>برای مثال:</i>  @username",parse_mode="html")
      step = "SaveGroup"
...
def SaveGroup(chat_id,message_id,text,message):
    ...
    db.close()
    print("در لیست ذخیره شد")
    app.send_message(chat_id,f"<code>گروه {text} در لیست گروه ها دخیره شد</code>",reply_to_message_id=message_id)
...
def RemoveGroup(chat_id,message_id,query):
      try:
            ...
            if len(listgroups) > 0:
                  ...
                  key = iter(keyboard)
                  Output = [list(islice(key,elem))for elem in[2 for n in range(len(keyboard))]]
                  Output.append([button(text="برگشت 🔙",callback_data="EditGroups")])

                  app.edit_message_text(chat_id,message_id,"<i>برای حذف گروه روی اسم گروه کلیک کنید</i> 👇🏼",
                  reply_markup=markup(Output))

            else:
                  query.answer(f"گروهی در لیست وجود ندارد")
                  StartSet(chat_id,message_id)
      except Exception as error:print(error)
...
def DeleteGroups(chat_id,message_id,query,data):
      try:
            ...
            db.delete_group(ID)
            db.close()
            query.answer(f"با موفقیت حذف شد")
...
def ShowGroup(chat_id,message_id,query):
      ...
      if len(listgroups) > 0:
            ...
            key = iter(keyboard)
            Output = [list(islice(key,elem))for elem in[2 for n in range(len(keyboard))]]
            app.edit_message_text(chat_id,message_id,"<i>لیست زیر گروه های ثبت شده میباشند</i> 👇🏼",reply_markup=markup(Output))
...
def EditAccount(chat_id,message_id):
      global step
      app.edit_message_text(chat_id,message_id,"<i>برای حذف اکانت روی گزینه پایین کلیک کنید</i> 👇🏼",
                              reply_markup=markup([[button("حذف اکانت ✖️",callback_data="RemoveAccount")],
                              [button("برگشت 🔙",callback_data="ComebackSettings")]]))
      step = 'None'

def RemoveAccount(chat_id,message_id,query):
      global step
      app.edit_message_text(chat_id,message_id,"شماره اکانت را ارسال کنید")
      step = "DeleteAccount"
...
def DeleteAccount(chat_id,message_id,text,message):
      ...
      try:
            ...
            db.close()
            app.send_message(chat_id,f"اکانت  <b>{text}</b>  با موفقیت از دیتابیس حذف شد ✔️",parse_mode='html',
                              reply_to_message_id=message_id)
...
def SettingAdd(chat_id,message_id):#SettingAdd
      ...
      app.edit_message_text(chat_id,message_id,"SettingAdd",
                              reply_markup=markup([[button("تنظیم کردن ⚙️",callback_data="EditSettings"),button("نمایش تنظیمات 👀",callback_data="ShowSettings")],
                              [button("برگشت 🔙",callback_data="ComebackSettings")]]))

def ShowSettings(chat_id,message_id,query):

      db = SettingAddmember("Add_GptoGp.db")
      db.create_table()
      ...
      if len(list_Settingss) >= 1:
            ...
            query.answer(f"تنظیم نشده ❌")
            StartSet(chat_id,message_id)

def EditSettings(chat_id,message_id):
      global step
      app.send_message(chat_id,"<code>ارسال کنید</code>\n<i>برای مثال(استراحت روزانه):</i>  <b>d=1</b>\n<i>برای مثال(استراحت ساعتی):</i>  <b>t=12</b>",
...
def GetActivityRange(chat_id,message_id,text,message):
      ...
      try:
            if text[0] == "t":
                  if int(text[2:]) < 24:
                        app.send_message(chat_id,"<code>تعداد ادد با هر اکانت</code>\n<i>برای مثال:</i>  <b>10 or 20 or 30 or..</b>",
                        parse_mode='html',reply_markup=reply(selective=True))
                        step = f"GetAddRange{text}"
                  else:
                        app.send_message(chat_id,"<b>تایم استراحت باید بین 1 الی 23 باشد</b>\n<b>دوباره ارسال کنید..</b>",
                  ...
                  if int(text[2:]) >= 1:
                        ...
                        app.send_message(chat_id,"<b>بالای یک روز لطفا</b>\n<b>دوباره ارسال کنید..</b>",
                        reply_markup=reply(selective=True))
                        step = "GetActivityRange"
            else:
                  app.send_message(chat_id,"<b>فرمت ارسالی شما اشتباه می باشد</b>\n<b>دوباره ارسال کنید..</b>",
                  parse_mode='html',reply_markup=reply(selective=True))
...
def GetAddRange(chat_id,message_id,text,message):
      ...
      if len(list_setting) < 1:
            ...
            db.close()
            app.send_message(chat_id,f"<code>✅ Your settings have been applied</code>",reply_to_message_id=message_id)
...
def Accounts(chat_id,message_id,query):
      ...
      if len(get_list_Setting) < 1:
            ...
            query.answer(f"❌ تنظیمات انجام نشده")
...
def Activityy(chat_id,message_id,query):
      ...
      global step
      db = AddStatistics("Add_GptoGp.db")
      db.create_table()
...
def StartUAR(chat_id,message_id):
      ...
      if len(list_Account) > 0:
            ...
            app.send_message(chat_id,"<code>To update the report status of accounts ... I do not see any accounts</code>",parse_mode='html',reply_to_message_id=message_id)
      db.close()
#/------------------------------------------------------------------------------------------------------------
...
def AddToGroups(chat_id,message_id,query,data):
      global step
      app.edit_message_text(chat_id,message_id,f"<code> How Many Accounts Do You want to work with? </code>\n<i>For example:</i>  <b>6</b> ",parse_mode="html")
...
def ForAddChatMember(chat_id,message_id,list_Account,usergroup):
      ...
      for account in list_Account:
            ...
            T=threading.Thread(target=AddMemberToGroup, args = [account,usergroup,chat_id,message_id])
            T.start()
            sleep(30)
...
def UpdateDeleteAccount(chat_id,message_id,account):
      ...
      db.close()
      app.send_message(chat_id,f"<b>This account[<code>{account}</code>] was deleted by Telegram</b>",parse_mode="html")

def MessageAddMember(chat_id,message_id,today_send_success,today_send_unsuccess,account):
      app.send_message(chat_id,f"<b>Account</b><code>{account}</code><b> did {today_send_success} successful and {today_send_unsuccess}  unsuccessful</b>",parse_mode="html")
...
app.start()
print("Developer: @Add_GptoGp      <<<< barname nevis in bot id roo be roo mibashad ")
